...
public class ShortHonorModeratorView extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		int i = 7;
		Collection<ParticipantJourney> participants = journey.getModerators();
		...
		for (ParticipantJourney participant : participants) {
			...
			getCell(sheet, i, 8).setCellValue(Constant.DECIMAL_FORMAT.format(participant.getModeratorAmount()) + " x " + participant.getDays() + " hari" + " x " + participant.getModeratorHours() + " jam");

			getCell(sheet, i, 9).setCellStyle(style);
			getCell(sheet, i, 9).setCellValue(participant.getModeratorTotalAmount().floatValue());

			getCell(sheet, i, 10).setCellStyle(style);
			getCell(sheet, i, 10).setCellValue(participant.getModeratorPpn().doubleValue());

			getCell(sheet, i, 11).setCellStyle(style);
			getCell(sheet, i, 11).setCellValue(participant.getModeratorReceivedAmount().floatValue());
