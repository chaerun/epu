...
public class ShortNormatifView extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		for (ParticipantJourney participant : participants) {
			...
			if (participant.isFlag()) {
				...
				if (participant.getParticipant().getCode().toLowerCase().startsWith("honorer")) {
					getCell(sheet, i, 3).setCellValue("-");
				} else {
		...
		for (Informant informant : informants) {
			...
			if (flag) {
				sheet.addMergedRegion(new CellRangeAddress(i, i, 1, 11));
