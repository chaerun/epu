...
public class ShortHonorNarasumberView extends AbstractExcelView {
	...
	protected void buildExcelDocument(Map<String, Object> model, HSSFWorkbook workbook, HttpServletRequest request, HttpServletResponse response) throws Exception {
		...
		for (Informant informant : informants) {
			...
			getCell(sheet, i, 8).setCellValue(Constant.DECIMAL_FORMAT.format(informant.getAmount()) + " x " + informant.getDays() + " hari" + " x " + informant.getHours()+ " jam");

			getCell(sheet, i, 9).setCellStyle(style);
			getCell(sheet, i, 9).setCellValue(informant.getAmount().floatValue());

			getCell(sheet, i, 10).setCellStyle(style);
			getCell(sheet, i, 10).setCellValue(informant.getTotalPpnAmount().doubleValue());
			
			
			BigDecimal total=informant.getTotalHonorAmount();
			BigDecimal totalPpn =informant.getTotalPpnAmount();
			BigDecimal receivedAmount = total.subtract(totalPpn);
			
			getCell(sheet, i, 11).setCellStyle(style);
			getCell(sheet, i, 11).setCellValue(receivedAmount.floatValue());
