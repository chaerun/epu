...
public class JourneyValidator implements Validator {
	...
	public void validate(Object target, Errors errors) {
		...
		ValidationUtils.rejectIfEmptyOrWhitespace(errors, "location", "field.required", "field.required");
		...
		if (JourneyType.FULLBOARD.equals(journey.getType()) || JourneyType.FULLDAY.equals(journey.getType())) {
			...
			journey.setPacketMeetingAmount(BigDecimal.ZERO);
			journey.setPacketMeetingAccount(null);
		}
		...
		if (JourneyType.JOURNEY_IN.equals(journey.getType()) || JourneyType.JOURNEY_OUT.equals(journey.getType())) {
			...
			journey.setInnAmount(BigDecimal.ZERO);
			journey.setInnAccount(null);
		}

		DateTime startDate = new DateTime(journey.getStartDate());
		DateTime endDate = new DateTime(journey.getEndDate());

		if (endDate.isBefore(startDate)) {
			errors.rejectValue("endDate", "date.endDate.limit");
			errors.rejectValue("startDate", "date.startDate.limit");
		}
