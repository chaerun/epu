			...
			<div class="panel-body">
				<display:table id="activity" class="table table-striped" name="page" sort="external" partialList="true" size="${page.fullListSize}" pagesize="${page.objectsPerPage}" requestURI="" export="true">
					<display:column titleKey="action">
						<a href="${pageContext.request.contextPath}/receipt/activity/${activity.code}/" class="btn btn-sm btn-info"><i class="fa fa-check-square-o"></i> <spring:message code="choose" /> </a>
					</display:column>
					...
					<display:column titleKey="absorption" sortable="true">
						<fmt:formatNumber value="${activity.usedAmount}" currencySymbol="Rp." pattern="¤ #,##0.00;¤ -#,##0.00" />
					</display:column>
					<display:column sortProperty="usedAmount" titleKey="remnant.amount" sortable="true">
						<fmt:formatNumber value="${activity.balance}" currencySymbol="Rp." pattern="¤ #,##0.00;¤ -#,##0.00" />
					</display:column>
